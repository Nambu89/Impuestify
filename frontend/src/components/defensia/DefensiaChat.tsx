...
export function DefensiaChat({ expedienteId }: Props) {
    ...
    return (
        <div className="defensia-chat">
            ...
            <div className="defensia-chat-messages" role="log">
                {messages.length === 0 && (
                    <p className="defensia-chat-empty">
                        Pregúntale a DefensIA sobre este expediente. Solo responderá sobre el caso
                        que tienes abierto.
                    </p>
                )}
                {messages.map((m, i) => (
                    <div key={i} className={`defensia-chat-msg defensia-chat-msg--${m.role}`}>
                        <div className="defensia-chat-bubble">
            ...
            {error && <div className="defensia-chat-error">{error}</div>}
