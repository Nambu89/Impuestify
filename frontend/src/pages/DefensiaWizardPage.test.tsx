...
describe('DefensiaWizardPage', () => {
    ...
    it('muestra DisclaimerBanner en todos los pasos', () => {
        renderPage()
        expect(screen.getByText(/no sustituye asesoramiento profesional/i)).toBeInTheDocument()
    })
    ...
    it("paso 5: click 'Analizar' dispara useDefensiaAnalyze con callbacks", async () => {
        ...
        await user.click(screen.getByRole('button', { name: /analizar expediente/i }))
    ...
    it('tras upload exitoso con fase_detectada, el paso 3 muestra la fase real', async () => {
        ...
        expect(screen.getByLabelText(/plazo de recurso/i)).toBeInTheDocument()
        expect(screen.queryByText(/analizando los documentos/i)).not.toBeInTheDocument()
    })
})
