...
import {
    DocumentoUploadCard,
    type UploadDocumento,
} from '../components/defensia/DocumentoUploadCard'
...
export function DefensiaWizardPage() {
    ...
    const handleSiguiente = async () => {
        if (state.paso === 1 && state.tributo !== null) {
            ...
            try {
                const res = await apiRequest<{ id: string }>('/api/defensia/expedientes', {
                    method: 'POST',
                    body: JSON.stringify({
        ...
        if (state.paso === 4 && state.expedienteId) {
            try {
                await apiRequest(`/api/defensia/expedientes/${state.expedienteId}/brief`, {
                    method: 'POST',
                    body: JSON.stringify({ texto: state.brief }),
                })
            } catch (err) {
                const msg = err instanceof Error ? err.message : 'Error al guardar el brief'
                dispatch({ type: 'CREAR_ERROR', error: msg })
                return
            }
            dispatch({ type: 'NEXT' })
        ...
        if (state.paso === 5 && state.expedienteId) {
            ...
            try {
                await analyze(state.expedienteId, {
                    ...
                    onCandidatos: (d) =>
                        setAnalyzeStatus(`Aplicando reglas (${d.count} candidatos)…`),
                    onVerificando: () =>
    ...
    return (
        <div className="defensia-wizard-page">
                            ...
                            <>
                                ...
                                {state.faseDetectada === 'INDETERMINADA' && (
                                    <p className="defensia-wizard-hint">
                                        ...
                                        continuar y describir el caso en el paso siguiente; el motor
                                        intentará deducirla del brief.
                                    </p>
                                )}
                            </>
                        ) : (
                ...
                {state.paso === 5 && (
                    <section aria-labelledby="paso5-title">
                        ...
                        {!analyzing && !analyzeStatus && (
                            <>
                                <p>Al pulsar "Analizar expediente" DefensIA hará lo siguiente:</p>
                                <ol>
            ...
            {state.error && <div className="defensia-wizard-error">{state.error}</div>}
