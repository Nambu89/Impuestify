...
export function useFeedback() {
    ...
    const submitChatRating = useCallback(
        async (data: {
            message_id: string
    ...
    const adminGetFeedback = useCallback(
        async (
            params: {
                type?: string
                status?: string
            ...
            return await apiRequest<{ items: FeedbackItem[]; total: number }>(
    ...
    const adminGetFeedbackDetail = useCallback(
        async (id: string): Promise<FeedbackItem & { screenshot_data?: string }> => {
            return await apiRequest(`/api/admin/feedback/${id}`)
        },
        [apiRequest],
    )

    const adminUpdateFeedback = useCallback(
        async (
            id: string,
            data: {
    ...
    const adminGetContactRequests = useCallback(
        async (
            params: {
                status?: string
                page?: number
            ...
            if (params.limit) qs.set('limit', String(params.limit))
            const query = qs.toString() ? `?${qs.toString()}` : ''
            return await apiRequest<{ items: ContactRequest[]; total: number }>(
    ...
    const adminUpdateContactRequest = useCallback(
        async (id: string, status: 'pending' | 'responded'): Promise<void> => {
    ...
    const adminGetChatRatings = useCallback(
        async (
            params: {
                rating?: string
                page?: number
            ...
            if (params.limit) qs.set('limit', String(params.limit))
            const query = qs.toString() ? `?${qs.toString()}` : ''
            return await apiRequest<{ items: ChatRatingItem[]; total: number }>(
                `/api/admin/chat-ratings${query}`,
            )
        },
        [apiRequest],
    )
