...
export function useApi() {
    const askQuestion = useCallback(
        async (question: string, conversationId?: string, k?: number): Promise<AskResponse> => {
            ...
            try {
                ...
                logger.debug('Question response received:', {
    ...
    const apiRequest = useCallback(
        async <T = any>(url: string, options?: RequestInit & { timeout?: number }): Promise<T> => {
