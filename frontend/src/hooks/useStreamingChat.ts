...
export const useStreamingChat = (): UseStreamingChatReturn => {
    ...
    const sendStreamingMessage = useCallback(
        async (
            ...
            setIsStreaming(true)
            logger.debug('SSE sendStreamingMessage called', {
                    ...
                    if (refreshToken) {
                        try {
                            ...
                            if (refreshRes.ok) {
                                ...
                                const newToken =
                                    refreshData.access_token || refreshData.tokens?.access_token
                                const newRefresh =
                                    refreshData.refresh_token || refreshData.tokens?.refresh_token
                                if (newToken) {
