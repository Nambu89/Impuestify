...
> Sesion 42 (2026-05-17): BoeApiClient (httpx async + parser XML), CitationEnricher (enriquece markdown con links a BOE consolidados), tabla boe_cache (UPSERT 30 días TTL), citation verifier extiende con `_check_derogated_norms` (env-gated `BOE_VERIFY_VIGENCIA`). Tests: 157 PASS (+32 nuevos: 16 boe_client + 16 citation_enricher). Citas inventadas se ignoran (test cubre regresión). Mapping `boe_id` a 22 entradas norms.yaml verificado contra BOE real. Frontend `MdAnchor` para links externos con icono ↗.
