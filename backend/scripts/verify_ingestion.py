...
async def verify_ingestion():
    ...
    print(
        f"  Average Chunks per Page: {s['total_chunks'] / max(s['unique_pages_with_chunks'], 1):.1f}"
    )
    print()
    ...
    for row in coverage.rows:
        ...
        print(
            f"  {status} {row['filename'][:50]:<50} {row['indexed_pages']:>4}/{row['total_pages']:<4} ({coverage_pct:>5.1f}%) - {row['chunk_count']} chunks"
        )
