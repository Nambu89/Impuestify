...
doc.add_paragraph(
    "Google te da resultados genericos, muchas veces desactualizados o de otros paises. Impuestify tiene 454 documentos oficiales indexados (AEAT, BOE, haciendas forales) con 89.000+ fragmentos de legislacion vigente. Las respuestas estan respaldadas por fuentes concretas, no por articulos de blogs."
)

doc.add_heading("Frente a preguntarle a ChatGPT:", level=2)
doc.add_paragraph(
    'ChatGPT no conoce tu CCAA, tu situacion laboral ni tus deducciones especificas. Inventa datos fiscales ("alucina"). Impuestify usa RAG (Retrieval-Augmented Generation) con documentacion oficial verificada, conoce los 21 territorios espanoles, y tiene herramientas de calculo real (simulador IRPF, calculadora de cuotas, comparador conjunta/individual). No inventa: calcula.'
)

doc.add_heading("Frente a usar otra web (TaxDown, Declarando...):", level=2)
doc.add_paragraph(
    "Las webs tipo TaxDown o Declarando son formularios rigidos que te hacen la declaracion por ti (y cobran por ello). Impuestify es un asistente inteligente que te explica, te orienta y te ayuda a entender. No compite con ellos: los complementa. Ademas, Impuestify cubre territorios forales (Pais Vasco, Navarra) que la mayoria de competidores ignoran."
)

doc.add_heading("Frente a ir a una gestoria:", level=2)
doc.add_paragraph(
    "Una gestoria cuesta 60-200 EUR por declaracion y solo te atiende en horario laboral. Impuestify esta disponible 24/7, responde en segundos, y cuesta 5 EUR/mes para particulares. No sustituye al asesor fiscal para casos complejos, pero resuelve el 80% de las dudas que la gente tiene antes de necesitar uno."
)
...
for t, d in [
    ...
    ("6. Dejar su email", "No tenemos newsletter por ahora. El registro ya captura el email."),
]:
    p = doc.add_paragraph()
    p.add_run(t).bold = True
    p.add_run(f"\n{d}")
...
doc.add_paragraph(
    "Sin duda, el chat con asistente fiscal IA. Le preguntas cualquier cosa sobre impuestos en Espana y te responde con datos concretos, cifras exactas y referencias legales, personalizado para tu CCAA y tu situacion."
)
doc.add_paragraph(
    'Ejemplo real: le preguntas "que epigrafe IAE necesito para ponedoras de huevos en Bizkaia" y te responde con el codigo exacto (019.3 - Avicultura de puesta), con la tabla de cuotas forales y la referencia al BOB. Ningun otro servicio hace esto.'
)
doc.add_paragraph(
    "Lo segundo mas potente es la guia fiscal interactiva: en 7 pasos tienes tu resultado IRPF estimado con deducciones autonomicas. Visual, rapido, sin tecnicismos."
)
...
for mat, desc in [
    (
        "Logo:",
        'Si. Escudo con cerebro IA + texto "Impuestify". Version horizontal para header, favicon para navegador.',
    ),
    (
        "Colores e imagen de marca:",
        "Si. Azul corporativo (#1e40af), cyan acento (#06b6d4), fondo blanco. Consistente en toda la web.",
    ),
    (
        "Textos de la web:",
        "Si. Landing completa, paginas de territorios (forales, Ceuta/Melilla, Canarias), pagina de creadores, politica privacidad, terminos, transparencia IA.",
    ),
    ("Capturas:", "Si. Se pueden generar facilmente de todas las secciones."),
    (
        "Videos:",
        "No. Pendiente. Podria hacer screencasts (grabacion de pantalla mostrando la herramienta).",
    ),
    (
        "Perfiles en redes:",
        "Si. LinkedIn + Instagram + TikTok creados. Contenido minimo por ahora.",
    ),
    (
        "Base de datos:",
        "Si. Usuarios registrados (beta testers activos), 454 documentos fiscales oficiales, 1.000+ deducciones.",
    ),
    ("Contactos:", "Si. 3 beta testers activos dando feedback real."),
    (
        "Testimonios:",
        "En proceso. Los beta testers estan usando la herramienta pero aun no tenemos testimonios formales.",
    ),
    (
        "Presupuesto para publicidad:",
        "Limitado. Estoy bootstrapping. Prefiero invertir en contenido organico y SEO antes que en ads pagados.",
    ),
]:
    p = doc.add_paragraph()
...
for t, d in [
    (
        "Tiempo semanal:",
        "Unas 5-8 horas semanales para marketing/contenido. El resto del tiempo lo dedico a desarrollo del producto.",
    ),
    (
        "Crear contenido:",
        "Si, puedo escribir posts y crear graficos basicos. Tengo conocimiento profundo del tema fiscal que es la mayor ventaja.",
    ),
    (
        "Grabar videos:",
        "Prefiero evitarlo por ahora. No me siento comodo delante de la camara. Si puedo hacer screencasts (grabacion de pantalla mostrando la herramienta).",
    ),
    (
        "Web vs redes sociales:",
        "Prefiero que el plan dependa mas de la web y del SEO. Las redes sociales como complemento, no como canal principal. El contenido de la web dura, el de redes desaparece.",
    ),
    (
        "Que no quiero hacer:",
        "No quiero hacer spam, no quiero cold outreach agresivo, no quiero publicar 3 veces al dia en redes. Prefiero calidad a cantidad.",
    ),
]:
    p = doc.add_paragraph()
    p.add_run(t).bold = True
...
doc.add_paragraph(
    "Impuestify tutea al usuario. Usa lenguaje natural, evita jerga fiscal innecesaria. Cuando usa terminos tecnicos los explica. Es directo: dato primero, explicacion despues."
)
doc.add_paragraph(
    "No es informal ni coloquial (los impuestos son cosa seria), pero tampoco es frio ni burocratico. Es como hablar con un amigo que sabe mucho de fiscalidad y te lo explica sin hacerte sentir tonto."
)
doc.add_paragraph(
    "Referencia de tono: como un asesor fiscal joven que te explica las cosas en un cafe, con cifras y referencias legales pero sin abrumarte."
)
...
output = os.path.join(
    ...
    "Impuestify_Plan_Marketing_Respuestas.docx",
)
doc.save(output)
