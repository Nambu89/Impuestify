...
class TableDetector:
    ...
    PATTERNS = {
        ...
        "retenciones": [r"retenc[ií]on|tipo.*retenc", r"rendimientos.*trabajo", r"tabla.*retenc"],
        "plazos": [r"plazo|fecha.*límite", r"modelo\s+\d+", r"presentac[ií]on"],
        "reducciones": [
            r"reducc[ií]on",
            r"rendimientos.*trabajo|rendimientos.*capital",
            r"importe|cuantía",
        ],
        "tipos_iva": [r"tipo.*iva|iva.*(?:general|reducido|superreducido)", r"21\s*%|10\s*%|4\s*%"],
    }
            ...
            if has_table:
                ...
                if "escala_irpf" in table_types and len(stats["sample_tables"]) < 2:
                    stats["sample_tables"].append(
                        {"page": page, "type": "escala_irpf", "preview": content[:200]}
                    )
            ...
            if has_significant_tables:
                ...
                if stats["table_types"]:
                    ...
                    for ttype, count in sorted(
                        stats["table_types"].items(), key=lambda x: x[1], reverse=True
                    ):
