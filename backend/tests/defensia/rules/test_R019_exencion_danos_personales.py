...
def _cargar_solo_R019() -> None:
    ...
    """
    reset_registry()
    module_name = "app.services.defensia_rules.reglas_irpf.R019_exencion_danos_personales"
...
def _assert_cita_no_hardcoded(cita: str) -> None:
    ...
    """
    assert "Art. 7.d" not in cita, f"Cita hardcoded detectada: 'Art. 7.d' en '{cita}'."
    assert "LO 1/2025" not in cita, f"Cita hardcoded detectada: 'LO 1/2025' en '{cita}'."
    assert "7.d LIRPF" not in cita.upper(), f"Cita hardcoded detectada: '7.d LIRPF' en '{cita}'."
    assert (
        "ley organica 1/2025" not in cita.lower()
    ), f"Cita hardcoded detectada: 'Ley Organica 1/2025' en '{cita}'."
    assert (
        "articulo 7.d" not in cita.lower()
...
def test_R019_positivo_indemnizacion_judicial_danos_personales(build_exp, build_brief, build_doc):
    ...
    assert (
        disparo.get("motivo") == "responsabilidad_civil_judicial"
    ), f"datos_disparo.motivo inesperado: {disparo!r}"
    assert disparo.get("importe") == 30000, f"datos_disparo.importe inesperado: {disparo!r}"
...
def test_R019_positivo_acuerdo_mediacion_MASC_dentro_baremo(build_exp, build_brief, build_doc):
    ...
    r019 = [c for c in candidatos if c.regla_id == "R019"]
    assert len(r019) == 1, (
        f"R019 deberia disparar con acuerdo MASC dentro del Baremo, " f"got {candidatos}"
    )

    arg = r019[0]
    _assert_cita_no_hardcoded(arg.cita_normativa_propuesta)
    cita_lower = arg.cita_normativa_propuesta.lower()
    assert "mediacion" in cita_lower or "masc" in cita_lower or "baremo" in cita_lower, (
    ...
    disparo = arg.datos_disparo
    assert (
        disparo.get("motivo") == "acuerdo_mediacion_MASC"
    ), f"datos_disparo.motivo inesperado: {disparo!r}"
...
def test_R019_positivo_aseguradora_en_cumplimiento_sentencia(build_exp, build_brief, build_doc):
    ...
    r019 = [c for c in candidatos if c.regla_id == "R019"]
    assert len(r019) == 1, (
        f"R019 deberia disparar con aseguradora en cumplimiento de sentencia, " f"got {candidatos}"
    )
    ...
    disparo = arg.datos_disparo
    assert (
        disparo.get("motivo") == "aseguradora_en_cumplimiento_sentencia"
    ), f"datos_disparo.motivo inesperado: {disparo!r}"
...
def test_R019_negativo_indemnizacion_laboral_pactada(build_exp, build_brief, build_doc):
    ...
    brief = build_brief(
        "Recibi una indemnizacion por despido pactado y Hacienda me la " "incluye como ingreso."
    )
...
def test_R019_negativo_masc_supera_baremo(build_exp, build_brief, build_doc):
    ...
    brief = build_brief("Acuerdo MASC por 30000 EUR cuando el Baremo aplicable era 18000 EUR.")

    candidatos = evaluar(exp, brief)

    r019 = [c for c in candidatos if c.regla_id == "R019"]
    assert r019 == [], (
        f"R019 NO debe disparar cuando el MASC supera el limite del Baremo, " f"got {r019}"
    )
...
def test_R019_cita_es_semantica_no_hardcoded(build_exp, build_brief, build_doc):
    ...
    assert (
        "Art. 7.d" not in cita and "LO 1/2025" not in cita and "7.d LIRPF" not in cita.upper()
...
def test_R019_registrada_en_registry():
    ...
    assert "R019" in REGISTRY, f"R019 no aparece en REGISTRY. Keys: {sorted(REGISTRY.keys())}"
    info = REGISTRY["R019"]
    assert "IRPF" in info["tributos"], f"R019 debe aplicar a IRPF, tributos={info['tributos']}"
    # La exencion art. 7.d LIRPF es especifica de IRPF — no aplica a IVA/ITP/etc.
    for tributo_fuera in ("IVA", "ISD", "ITP", "PLUSVALIA"):
        assert tributo_fuera not in info["tributos"], f"R019 NO deberia aplicar a {tributo_fuera}"
