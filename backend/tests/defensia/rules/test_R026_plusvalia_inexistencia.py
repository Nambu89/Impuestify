...
_R026_MODULE_NAME = (
    "app.services.defensia_rules.reglas_otros_tributos." "R026_plusvalia_inexistencia"
)
...
def test_R026_positivo_perdida_acreditada_escrituras(build_exp, build_brief, build_doc):
    ...
    assert (
        "incremento" in cita_lower or "inexistencia" in cita_lower or "no sujecion" in cita_lower
    ), (
    ...
    assert (
        arg.datos_disparo.get("perdida") == 30000
...
def test_R026_positivo_valores_iguales_cero_incremento(build_exp, build_brief, build_doc):
    ...
    brief = build_brief(
        "Vendi el piso por el mismo precio que lo compre y aun asi me cobran " "plusvalia."
    )
    ...
    assert (
        arg.datos_disparo.get("perdida") == 0
    ), f"datos_disparo.perdida inesperado: {arg.datos_disparo!r}"
...
def test_R026_negativo_incremento_positivo(build_exp, build_brief, build_doc):
    ...
    assert candidatos == [], f"R026 no deberia disparar con incremento positivo, got: {candidatos}"
...
def test_R026_negativo_valores_no_disponibles(build_exp, build_brief, build_doc):
    ...
    assert candidatos == [], (
        f"R026 no deberia disparar sin ambos valores de escritura, got: " f"{candidatos}"
    )
...
def test_R026_negativo_tributo_no_plusvalia(build_exp, build_brief, build_doc):
    ...
    assert candidatos == [], (
        f"R026 no deberia disparar sobre tributos distintos de PLUSVALIA, " f"got: {candidatos}"
    )
