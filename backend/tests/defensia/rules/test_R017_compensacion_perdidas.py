...
def _cargar_solo_R017() -> None:
    ...
    reset_registry()
    module_name = "app.services.defensia_rules.reglas_irpf.R017_compensacion_perdidas"
...
def _assert_cita_no_hardcoded(cita: str) -> None:
    ...
    assert (
        "art. 49 lirpf" not in cita.lower()
    ), f"Cita hardcoded detectada: 'art. 49 LIRPF' en '{cita}'."
    assert (
        "articulo 49" not in cita.lower()
    ), f"Cita hardcoded detectada: 'articulo 49' en '{cita}'."
    assert "art. 48" not in cita.lower(), f"Cita hardcoded detectada: 'art. 48' en '{cita}'."
    assert "lirpf 49" not in cita.lower(), f"Cita hardcoded detectada: 'LIRPF 49' en '{cita}'."
...
def test_R017_positivo_perdidas_pendientes_no_compensadas(build_exp, build_brief, build_doc):
    ...
    assert "perdidas" in cita_lower, (
        f"La cita semantica debe mencionar 'perdidas', got: " f"{arg.cita_normativa_propuesta!r}"
    )
    ...
    assert (
        disparo.get("perdidas_pendientes_total") == 5000
...
def test_R017_positivo_parcialmente_compensadas_bajo_limite(build_exp, build_brief, build_doc):
    ...
    disparo = arg.datos_disparo
    assert (
        disparo.get("perdidas_pendientes_total") == 8000
    ), f"perdidas_pendientes_total inesperado: {disparo!r}"
    assert disparo.get("compensado_actual") == 1000, f"compensado_actual inesperado: {disparo!r}"
    assert disparo.get("limite_aplicable") == 3000, f"limite_aplicable inesperado: {disparo!r}"
    # Delta: lo que AEAT dejo de aplicar dentro del margen permitido.
    assert disparo.get("delta_no_aplicado") == 2000, f"delta_no_aplicado inesperado: {disparo!r}"
...
def test_R017_negativo_compensacion_al_maximo(build_exp, build_brief, build_doc):
    ...
    liquidacion = _liquidacion_con_datos(build_doc, datos_liquidacion, doc_id="doc-liq-R017-maximo")
    exp = build_exp(
        tributo=Tributo.IRPF,
        fase=Fase.LIQUIDACION_FIRME_PLAZO_RECURSO,
        docs=[liquidacion],
    ...
    r017 = [c for c in candidatos if c.regla_id == "R017"]
    assert r017 == [], f"R017 NO debe disparar si compensacion == limite, got {r017}"
...
def test_R017_negativo_sin_perdidas_pendientes(build_exp, build_brief, build_doc):
    ...
    liquidacion = _liquidacion_con_datos(build_doc, datos_liquidacion, doc_id="doc-liq-R017-vacio")
    exp = build_exp(
        tributo=Tributo.IRPF,
        fase=Fase.LIQUIDACION_FIRME_PLAZO_RECURSO,
        docs=[liquidacion],
    ...
    r017 = [c for c in candidatos if c.regla_id == "R017"]
    assert r017 == [], f"R017 NO debe disparar sin perdidas pendientes, got {r017}"
...
def test_R017_negativo_perdidas_caducadas_fuera_plazo(build_exp, build_brief, build_doc):
    ...
    brief = build_brief("Tengo una perdida del 2019 que me gustaria compensar en 2024.")

    candidatos = evaluar(exp, brief)

    r017 = [c for c in candidatos if c.regla_id == "R017"]
    assert r017 == [], f"R017 NO debe disparar con perdidas caducadas (>4 anos), got {r017}"
...
def test_R017_cita_es_semantica_no_hardcoded(build_exp, build_brief, build_doc):
    ...
    assert (
        "Art. 49" not in cita and "art. 49 LIRPF" not in cita.lower()
...
def test_R017_registrada_en_registry():
    ...
    info = REGISTRY["R017"]
    assert "IRPF" in info["tributos"], f"R017 debe aplicar a IRPF, tributos={info['tributos']}"
