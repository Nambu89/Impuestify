...
class IRPFCalculator:
    ...
    async def calculate_irpf(
        ...
        if include_state:
            try:
                ...
                cuota_estatal, breakdown_estatal = self._apply_scale(base_liquidable, state_scale)
            except ValueError:
        ...
        cuota_autonomica, breakdown_autonomica = self._apply_scale(base_liquidable, ccaa_scale)
    ...
    def calculate_with_custom_scale(
        ...
        if tramos_estatales:
            cuota_estatal, breakdown_estatal = self._apply_scale(base_liquidable, tramos_estatales)

        # Calculate CCAA quota with custom scale
        cuota_autonomica, breakdown_autonomica = self._apply_scale(
            base_liquidable, tramos_autonomicos
        )
    ...
    def format_result(self, result: dict) -> str:
        ...
        if result["breakdown_estatal"]:
            for tramo in result["breakdown_estatal"][-3:]:  # Show last 3 brackets
                lines.append(
                    f"   • Tramo {tramo['tramo']}: {tramo['base_gravada']:,.2f} € × {tramo['tipo']}% = {tramo['cuota']:,.2f} €"
                )
        lines.append("")
...
async def test_calculator():
    ...
    try:
        result = await calc.calculate_irpf(base_liquidable=35000, jurisdiction="Aragon", year=2024)
