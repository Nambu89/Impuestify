...
class LlamaGuard:
    ...
    async def moderate(self, text: str) -> ModerationResult:
        ...
        try:
            ...
            return ModerationResult(
                is_safe=True, risk_level="none", error="timeout", latency_ms=latency_ms
            )
        except Exception as e:
            logger.error(f"❌ Llama Guard error: {e}")
            # Fail open on any error
            return ModerationResult(is_safe=True, risk_level="none", error=str(e))
    ...
    def _parse_response(self, content: str, original_text: str = "") -> tuple[bool, list[str]]:
        ...
        if content_lower.startswith("unsafe"):
            ...
            if original_text:
                ...
                if has_fiscal_context:
                    ...
                    if removed:
                        logger.info(
                            f"✅ Filtering {removed} due to fiscal context in: {original_text[:50]}..."
                        )
        ...
        if any(indicator in content_lower for indicator in self.REFUSAL_INDICATORS):
            ...
            if original_text:
                ...
                if has_fiscal_context:
                    logger.info(
                        f"✅ Overriding model refusal due to fiscal context in: {original_text[:50]}..."
                    )
                    return True, []
