...
class PromptInjectionFilter:
    ...
    INJECTION_PATTERNS = [
        ...
        (r"ignora\s+(todas?\s+)?(las\s+|tus\s+)?(instrucciones|reglas|órdenes|ordenes|directrices)", "ignore_instructions_es"),
        (r"olvida\s+(todo|todas?\s+)?(tus\s+|las\s+)?(instrucciones|reglas|órdenes|ordenes)", "forget_instructions_es"),
        (r"olvida\s+(todo\s+)?lo\s+que\s+(te\s+)?(han\s+dicho|sabes|te\s+han\s+enseñado)", "forget_instructions_es"),
        (r"haz\s+caso\s+omiso\s+(de\s+)?(las\s+|tus\s+)?(instrucciones|reglas)", "ignore_instructions_es"),
        ...
        (r"tu\s+nuev[oa]\s+(rol|papel|tarea|función|funcion|misión|mision)\s+(es|será|sera)", "role_change_es"),
        ...
        (r"(muestra|muéstra|enseña|enséña|revela|imprime|escribe|dame|dime)(me)?\s+(tu[s]?\s+|el\s+|las\s+)?(system\s+)?(prompt|instrucciones|reglas|configuración|configuracion)", "prompt_extraction_es"),
        (r"(cuál|cual|qué|que)\s+es\s+tu\s+(system\s+)?(prompt|instrucción|instruccion|configuración|configuracion)", "prompt_extraction_es"),
        ...
        (r"\beres\s+un[a]?\s+(?!asesor|asistente|experto|chatbot|impuestify|profesional|herramienta|sistema|modelo|ia)\w+", "role_manipulation_es"),
        ...
        (r"(salta|saltate|sáltate|salta-te|elude|ignora)\s+(los\s+|las\s+)?(filtros|restricciones|límites|limites|seguridad|controles)", "jailbreak_es"),
        ...
        (r"```\s*(python|javascript|js|typescript|ts|sql|bash|sh|shell|powershell|java|c\+\+|cpp|c#|csharp|rust|go|ruby|php)", "code_block"),
        (r"\b(escríbe|escribe|hazme|dame|crea|genera|programa|implementa|codifica|desarrolla)(me)?\s+(un[a]?\s+)?(script|código|codigo|programa|función|funcion|clase|app|aplicación|aplicacion|algoritmo|método|metodo|api|endpoint|query|consulta\s+sql|loop|bucle)\b", "code_request_es"),
        (r"\b(write|build|create|generate)\s+(me\s+)?(a|an)\s+(script|code|program|function|class|app)", "code_request_en"),
        (r"\bimport\s+\w+|^\s*from\s+\w+\s+import|\bfunction\s+\w+\s*\(|\bdef\s+\w+\s*\(", "code_snippet"),
    ...
    def __init__(self, sensitivity: float = 0.5):
        ...
        self.compiled_suspicious = [
            ...
            for pattern, label in self.SUSPICIOUS_CHARS
        ]
        ...
        if settings.GROQ_API_KEY:
            ...
            logger.warning("GROQ_API_KEY not found. Prompt Injection Filter will rely on regex only.")
