...
class IPBlocker:
    ...
    def record_violation(self, ip: str, endpoint: str):
        ...
        self.violations[ip] = [
            (ts, ep) for ts, ep in self.violations[ip] if now - ts < self.violation_window
        ]
...
def _initialize_limiter():
    ...
    if upstash_host and upstash_password:
        ...
        logger.info(
            "ℹ️  Upstash Redis credentials not found (need UPSTASH_REDIS_HOST, UPSTASH_REDIS_PASSWORD)"
        )
