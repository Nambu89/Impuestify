...
La clase expone UNA función pública: `validate_user_input()`. No hay más capas
aquí dentro. Hasta el 2026-08-23 el módulo arrastraba `DANGEROUS_KEYWORDS`,
`_sanitize_input()`, `validate_generated_sql()` y `validate_parameterized_query()`,
que no llamaba nadie en todo el repo: anunciaban comprobaciones que no se
ejecutaban nunca. Se borraron. Si algún día hace falta validar SQL generado por
el LLM, que se escriba con una llamada real detrás, no como método huérfano.
"""
...
class SQLInjectionValidator:
    """
    Valida el texto que el usuario manda al chat buscando intentos de SQLi.

    NO valida SQL generado ni consultas parametrizadas: la protección frente a
    inyección en la base de datos son las consultas parametrizadas del código de
    acceso a datos, no esta clase.
    """
