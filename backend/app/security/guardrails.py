...
class TaxIAGuardrails:
    ...
    def validate_input(self, user_question: str) -> GuardrailsResult:
        ...
        return GuardrailsResult(
            is_safe=is_safe, risk_level=risk_level, violations=violations, suggestions=suggestions
        )
    ...
    def validate_output(
        ...
        for pattern in self.COMPILED_DRIFT:
            if pattern.search(llm_response):
                ...
                logger.error(
                    f"OUTPUT DRIFT BLOCKED: pattern={pattern.pattern[:60]} response_preview={llm_response[:120]!r}"
                )
                risk_level = "critical"
                suggestions.append(
        ...
        has_grounding = any(
            indicator in llm_response.lower() for indicator in self.GROUNDING_INDICATORS
        )

        if not has_grounding and sources:
            violations.append("Response lacks proper source attribution")
            suggestions.append(
                "Add references like 'según la normativa...' or 'el artículo X establece...'"
            )
            risk_level = "medium"
        ...
        contains_risk_topic = any(keyword in lower_response for keyword in self.RISK_KEYWORDS)
        ...
        if contains_risk_topic and not disclaimer_present and self.strict_mode:
            violations.append(
                "Response discusses sanctions/penalties without professional disclaimer"
            )
            suggestions.append(
        ...
        uncertainty_count = sum(1 for phrase in hallucination_phrases if phrase in lower_response)

        if uncertainty_count > 2:
            violations.append(
                f"High uncertainty in response ({uncertainty_count} uncertain phrases)"
            )
    ...
    def apply_safety_wrapper(self, response: str, risk_level: str = "none") -> str:
        """
