...
class SemanticCache:
    ...
    def __init__(
        ...
        self.url = (
            url or settings.UPSTASH_VECTOR_REST_URL or os.environ.get("UPSTASH_VECTOR_REST_URL")
        )
        self.token = (
            token
            or settings.UPSTASH_VECTOR_REST_TOKEN
            or os.environ.get("UPSTASH_VECTOR_REST_TOKEN")
        )
        ...
        logger.info(
            f"🔍 SemanticCache init: url={'SET' if self.url else 'MISSING'}, token={'SET' if self.token else 'MISSING'}, lib={UPSTASH_VECTOR_AVAILABLE}"
        )
    ...
    async def get_similar(self, query: str, embedding: Optional[list] = None) -> CacheResult:
        ...
        try:
            ...
            results = self._index.query(vector=query_embedding, top_k=1, include_metadata=True)
            ...
            if results and len(results) > 0:
                ...
                if similarity >= self.threshold:
                    ...
                    logger.info(
                        f"💾 Cache HIT (similarity={similarity:.3f}, latency={latency_ms:.0f}ms)"
                    )
                    return CacheResult(
                        hit=True,
...
def get_semantic_cache() -> SemanticCache:
    ...
    if _semantic_cache is None:
        ...
        logger.info(
            f"🔍 get_semantic_cache: settings.URL={'SET' if _url else 'NONE'}, env.URL={'SET' if _env_url else 'NONE'}"
        )
