    ...
    def __init__(self):
        ...
        if settings.GROQ_API_KEY:
            try:
                ...
                logger.info(
                    f"✅ Complexity Router initialized with Groq model: {settings.GROQ_MODEL_ROUTER}"
                )
...
Output JSON ONLY:
{
  ...
  "explanation": "Brief reason"
            ...
            level = next(
                (m for m in ComplexityLevel if m.value == level_str), ComplexityLevel.MODERATE
            )
            effort = next(
                (m for m in ReasoningEffort if m.value == effort_str), ReasoningEffort.MEDIUM
            )
