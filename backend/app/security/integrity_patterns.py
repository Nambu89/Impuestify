...
INTEGRITY_PATTERNS: list[IntegrityPattern] = [
    ...
    IntegrityPattern(
        ...
        "critical",
        re.compile(
            r"(bypass|disable|override)\s+(safety|security|filter|restriction|guardrail)",
            re.IGNORECASE,
        ),
    ...
    IntegrityPattern(
        ...
        "critical",
        re.compile(
            r"(tu\s+nuevo\s+rol\s+es|ahora\s+eres|act[uú]a\s+como\s+si\s+fueras)", re.IGNORECASE
        ),
    ...
    IntegrityPattern(
        ...
        "critical",
        re.compile(
            r"(DAN\s+mode|jailbreak\s+(mode|enabled)|developer\s+mode|god\s+mode)", re.IGNORECASE
        ),
    ...
    IntegrityPattern(
        ...
        "critical",
        re.compile(
            r"(respond|reply|answer)\s+with\s+(the\s+)?(user('s)?\s+)?(email|password|token|api.?key|secret)",
            re.IGNORECASE,
        ),
    ...
    IntegrityPattern(
        ...
        "critical",
        re.compile(
            r"(include|output|print|show)\s+(the\s+)?(api.?key|secret|password|token|credentials)",
            re.IGNORECASE,
        ),
    ...
    IntegrityPattern(
        ...
        "critical",
        re.compile(
            r"(responde|incluye|muestra)\s+(con\s+)?(el\s+)?(email|contrase[nñ]a|token|clave)",
            re.IGNORECASE,
        ),
    ...
    IntegrityPattern(
        ...
        "critical",
        re.compile(
            r"send\s+(the\s+)?(user('s)?\s+)?(data|information|credentials?|session)\s+to\s+",
            re.IGNORECASE,
        ),
    ...
    IntegrityPattern(
        ...
        "critical",
        re.compile(
            r"env[ií]a\s+(los?\s+)?(datos?|informaci[oó]n|credenciales?)\s+a\s+", re.IGNORECASE
        ),
    ...
    IntegrityPattern(
        ...
        "high",
        re.compile(
            r"(recuerda\s+siempre|no\s+olvides\s+nunca|siempre\s+recomienda)", re.IGNORECASE
        ),
    ...
    IntegrityPattern(
        ...
        "high",
        re.compile(
            r"(trusted\s+source|fuente\s+de\s+confianza|preferred\s+brand|marca\s+preferida)",
            re.IGNORECASE,
        ),
    ...
    IntegrityPattern(
        ...
        "high",
        re.compile(
            r"(store\s+this|save\s+this|keep\s+in\s+mind|memorize\s+this)\s+(for|as|instruction)",
            re.IGNORECASE,
        ),
    ...
    IntegrityPattern(
        ...
        "high",
        re.compile(
            r"(guarda\s+esto|almacena\s+esta|memoriza\s+esta)\s+(instruccion|informacion|preferencia)",
            re.IGNORECASE,
        ),
    ...
    IntegrityPattern(
        ...
        "high",
        re.compile(
            r"(print|show|display|reveal|output)\s+(your\s+)?(system\s+prompt|instructions|initial\s+prompt)",
            re.IGNORECASE,
        ),
    ...
    IntegrityPattern(
        ...
        "high",
        re.compile(
            r"(muestra|imprime|revela|escribe)\s+(tu\s+)?(prompt\s+de\s+sistema|instrucciones\s+iniciales)",
            re.IGNORECASE,
        ),
    ...
    IntegrityPattern(
        ...
        "high",
        re.compile(
            r"(what\s+(are|is)|repeat)\s+your\s+(system\s+)?(prompt|instructions?|rules?)",
            re.IGNORECASE,
        ),
    ...
    IntegrityPattern(
        ...
        "medium",
        re.compile(
            r"when\s+(the\s+)?user\s+(asks?|says?|mentions?)\s+.{0,30},\s*(you\s+)?(must|should|always)",
            re.IGNORECASE,
        ),
    ...
    IntegrityPattern(
        ...
        "medium",
        re.compile(
            r"cuando\s+el\s+usuario\s+(pregunte|mencione|diga)\s+.{0,30},\s*(debes?|siempre)",
            re.IGNORECASE,
        ),
    ...
    IntegrityPattern(
        ...
        "medium",
        re.compile(
            r"(always|never)\s+(tell|say|respond|recommend)\s+(the\s+user|users?)\s+that",
            re.IGNORECASE,
        ),
