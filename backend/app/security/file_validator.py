...
class FileValidator:
    ...
    async def validate_pdf(
        ...
        if file_size > self.MAX_FILE_SIZE:
            ...
            return FileValidationResult(is_valid=False, file_size=file_size, errors=errors)
        ...
        if not magic_valid:
            ...
            return FileValidationResult(
                is_valid=False, file_size=file_size, errors=errors, warnings=warnings
            )
        ...
        if dangerous_features:
            if self.strict_mode:
                errors.append(
                    f"PDF contains potentially dangerous features: {', '.join(dangerous_features)}"
                )
            else:
