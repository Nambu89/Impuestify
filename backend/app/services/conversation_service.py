...
class ConversationService:
    ...
    async def add_message(
        ...
        await self.db.execute(sql, [message_id, conversation_id, role, content, metadata_json, now])
    ...
    async def add_message_sources(self, message_id: str, sources: list[dict[str, Any]]) -> None:
        ...
        try:
            result = await self.db.execute(
                f"SELECT id FROM document_chunks WHERE id IN ({placeholders})", chunk_ids
            )
        ...
        for idx, source in enumerate(valid_sources):
            ...
            params_list.append([source_id, message_id, chunk_id, source.get("score"), idx])

        if skipped:
            logger.warning(
                f"Skipped {skipped} sources with non-existent chunk_ids for message {message_id}"
            )
        ...
        if params_list:
            ...
            logger.info(
                f"No valid sources to insert for message {message_id} (all chunk_ids missing from DB)"
            )
