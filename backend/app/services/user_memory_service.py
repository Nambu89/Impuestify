...
class UserMemoryService:
    ...
    def __init__(
        ...
        if UPSTASH_VECTOR_AVAILABLE and self.vector_url and self.vector_token:
            try:
                ...
                logger.info(
                    "🧠 User Memory Service initialized with Upstash Vector + OpenAI embeddings"
                )
    ...
    def _extract_donation_type(self, message: str) -> str | None:
        ...
        if any(
            kw in message_lower
            for kw in ["dinero", "metálico", "metalico", "efectivo", "transferencia", "€", "euros"]
        ):
            return "money"
        elif any(
            kw in message_lower for kw in ["vivienda", "piso", "casa", "inmueble", "propiedad"]
        ):
            return "property"
    ...
    def _extract_donor_relation(self, message: str) -> str | None:
        ...
        if "mi madre" in message_lower or "madre" in message_lower:
            ...
            "mi pareja" in message_lower or "pareja" in message_lower or "cónyuge" in message_lower
        ):
            return "partner"
    ...
    def extract_facts_from_message(self, message: str) -> list[UserFact]:
        ...
        if any(kw in message_lower for kw in ["autónomo", "autonomo", "facturo", "mi negocio"]):
            ...
            kw in message_lower
            for kw in ["asalariado", "mi empresa", "mi jefe", "nómina", "nomina"]
        ):
            facts.append(
                UserFact(
        ...
        if donation_type:
            type_label = (
                ...
                else "vivienda/inmueble"
                if donation_type == "property"
                else "herencia"
            )
            facts.append(
                UserFact(
        ...
        if self.db:
            ...
            if profile:
                ...
                if datos_fiscales_raw:
                    try:
                        datos_fiscales = (
                            json.loads(datos_fiscales_raw)
                            if isinstance(datos_fiscales_raw, str)
                            else datos_fiscales_raw
                        )
                        for df_key in (
                            "edad",
                            "ingresos_brutos",
                        ...
                        if ingresos_trabajo_entry:
                            manual_value = None
                            if (
                                isinstance(ingresos_trabajo_entry, dict)
                                and "value" in ingresos_trabajo_entry
                            ):
    ...
    async def _update_datos_fiscales(
        self, user_id: str, key: str, value: Any, source: str = "conversation"
    ) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                "Updated datos_fiscales for user %s: %s=%s (source=%s)", user_id, key, value, source
            )
            return True
