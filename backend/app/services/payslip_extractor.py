...
class PayslipExtractor:
    ...
    def _parse_payslip_data(self, text: str) -> Dict:
        ...
        if m:
            ...
            if m2:
                ...
                m3 = re.search(
                    r"\b(" + "|".join(MONTH_MAP.keys()) + r")\s+(\d{4})\b", text, re.IGNORECASE
                )
                if m3:
        ...
        m = re.search(r"(\b[A-ZÀ-Ú][A-ZÀ-Ú0-9\s&\-]{1,40}?" + corp_suffixes + r")\b", text)
        if m:
        ...
        m = re.search(
            r"(?:Nº?\s*INS\.?\s*S\.?S\.?|C\.?C\.?C\.?)[\s:]*(\d{2}/\d{5,10}-\d{2})",
            text,
            re.IGNORECASE,
        )
        if m:
        ...
        m = re.search(
            r"(?:AFILIACI[OÓ]N|Nº?\s*S\.?S\.?)[\s.:]*(\d{2}/\d{8,10}-\d{2})", text, re.IGNORECASE
        )
        if m:
        ...
        if "employee_name" not in data:
            m = re.search(
                r"([A-ZÀ-Ú]{2,}(?:\s+[A-ZÀ-Ú]{2,})*,\s*[A-ZÀ-Ú]{2,}(?:\s+[A-ZÀ-Ú]\.?)*)\s", text
            )
            if m:
        ...
        if "gross_salary" not in data:
            ...
            m = re.search(
                r"(?:TOTAL\s+DEVENGADO|DEVENGOS\s+TOTALES|TOTAL\s+HABERES)\s+" + _NUM,
                text,
                re.IGNORECASE,
            )
            if m:
        ...
        irpf_pattern = re.compile(
            r"(?:TRIBUTACION\s+)?I\.?R\.?P\.?F\.?\s*(\d{1,2},\d{2})\s+" + _NUM, re.IGNORECASE
        )
        irpf_matches = list(irpf_pattern.finditer(text))
        if irpf_matches:
            data["irpf_percentage"] = float(irpf_matches[0].group(1).replace(",", "."))
            total_irpf = sum(self._parse_spanish_number(m.group(2)) or 0 for m in irpf_matches)
        ...
        cc_amounts = self._find_all_amounts(
            re.compile(r"COTIZACION\s+CONT\.?\s*COMU\.?\s+\d{1,2},\d{2}\s+" + _NUM, re.IGNORECASE),
            text,
        )
        if cc_amounts:
        ...
        de_amounts = self._find_all_amounts(
            re.compile(r"(?:COTIZACION\s+)?DESEMPLEO\s+\d{1,2},\d{2}\s+" + _NUM, re.IGNORECASE),
            text,
        )
        if de_amounts:
        ...
        if ss_total > 0:
            ...
            m = re.search(
                r"(?:contingencias\s+comunes|aportaci[oó]n\s+trabajador)[\s:]*" + _NUM,
                text,
                re.IGNORECASE,
            )
            if m:
        ...
        # --- 9. DATOS BANCARIOS ---
        m = re.search(
            r"IBAN[\s:]*([A-Z]{2}\d{2}\s*\d{4}\s*\d{4}\s*\d{4}\s*\d{4}\s*\d{4})",
            text,
            re.IGNORECASE,
        )
        if m:
        ...
        emp_cc = re.search(
            r"Contingencias\s+comunes.*?" + _NUM + r"\s+" + _NUM, text, re.IGNORECASE
        )
        if emp_cc:
        ...
        emp_mei = re.search(
            r"Equidad\s+Intergeneracional.*?" + _NUM + r"\s+" + _NUM + r"\s+" + _NUM,
            text,
            re.IGNORECASE,
        )
        if emp_mei:
        ...
        emp_des = re.search(
            r"Desempleo\.*\s+" + _NUM + r"\s+" + _NUM + r"\s+" + _NUM, text, re.IGNORECASE
        )
        if emp_des:
            data["employer_desempleo"] = self._parse_spanish_number(emp_des.group(3))

        emp_fogasa = re.search(
            r"Garant[ií]a\s+Salarial.*?" + _NUM + r"\s+" + _NUM, text, re.IGNORECASE
        )
        if emp_fogasa:
    ...
    def anonymize_text(text: str) -> str:
        ...
        result = re.sub(
            r"[A-Z]{2}\d{2}\s*\d{4}\s*\d{4}\s*\d{4}\s*\d{4}\s*\d{4}", "[IBAN_REDACTED]", result
        )

        # SWIFT/BIC: only after "SWIFT" or "BIC" keyword
        result = re.sub(
            r"(?:SWIFT|BIC)[/:\s]*([A-Z]{4}[A-Z]{2}[A-Z0-9]{2,5})",
            r"SWIFT [SWIFT_REDACTED]",
            result,
        )
        ...
        # Email addresses
        result = re.sub(
            r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b", "[EMAIL_REDACTED]", result
        )
