...
async def search_tax_regulations_tool(
    ...
    try:
        logger.info(
            f"Searching tax regulations: query='{query}', year={year}, extract_data={extract_data}"
        )
            ...
            if ccaa:
                ...
                for result in sorted_results:
                    ...
                    extracted = await extract_irpf_data_from_url(
                        url=result["url"], year=year, ccaa=ccaa
                    )

                    if extracted and extracted.get("success"):
                        tramos = extracted.get("tramos", [])
                        logger.info(
                            f"✅ Successfully extracted {len(tramos)} tramos from {result['source']}"
                        )
...
Estos datos se usarán para calcular tu IRPF.""",
        ...
        formatted_results = "\n".join(
            [f"• {r['title']} ({r['source']}): {r['url']}" for r in results[:max_results]]
        )
