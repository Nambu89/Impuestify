...
La función calcula la cuota mensual exacta de autónomos en España para 2025 según los ingresos netos mensuales y la región (general, Ceuta, o Melilla). Devuelve el tramo de cotización, la cuota mínima y máxima, y las bonificaciones aplicables.""",
        "parameters": {
            ...
            "properties": {
                ...
                "year": {"type": "integer", "description": "Año de cotización. Por defecto: 2026"},
                "tarifa_plana": {
                    "type": "boolean",
...
async def calculate_autonomous_quota_tool(
    ...
    if tarifa_plana:
        region_name = {"general": "España", "ceuta": "Ceuta", "melilla": "Melilla"}.get(
            region, region
        )
        return {
            "success": True,
...
📖 **Normativa**: Disposición adicional 52ª LGSS (RDL 13/2022, de 26 de julio)""",
    ...
    try:
        ...
        # Connect to database
        db = TursoClient(
            url=os.environ.get("TURSO_DATABASE_URL"), auth_token=os.environ.get("TURSO_AUTH_TOKEN")
        )
        await db.connect()
        ...
        result = await db.execute(
            sql, [year, region, ingresos_netos_mensuales, ingresos_netos_mensuales]
        )
