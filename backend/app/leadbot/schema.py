"""Esquema de tablas del leadbot — idempotente, sólo si LEADBOT_ENABLED.

Se invoca desde ``main.py`` en el lifespan, gateado por
``settings.LEADBOT_ENABLED``, de modo que estas tablas no se crean en las bases
de datos de los despliegues que no usan el captador. No tocamos
``turso_client.init_schema()`` para mantener el core intacto.

Todas las tablas llevan prefijo ``leadbot_`` para que convivan sin colisionar
con el esquema del producto.
"""
