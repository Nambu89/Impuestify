...
class CoordinatorAgent:
    ...
    def __init__(self):
        ...
        chat_client = OpenAIChatClient(model_id=settings.OPENAI_MODEL)
...
Responde SOLO con: "TaxAgent", "PayslipAgent", "WorkspaceAgent" o "CompetitorAnalysisAgent".""",
        ...
        logger.info(
            "CoordinatorAgent initialized with TaxAgent, PayslipAgent, WorkspaceAgent, and CompetitorAnalysisAgent"
        )
    ...
    async def run(self, query: str, context: dict[str, Any] = None):
        ...
        if context and context.get("payslip_data"):
            ...
            return await self.payslip_agent.analyze(
                payslip_data=context["payslip_data"], user_question=query
            )
        ...
        if agent_name == "PayslipAgent":
            ...
            sources = context.get("sources") if context else None
            return await self.tax_agent.run(query=query, context=rag_context, sources=sources)
