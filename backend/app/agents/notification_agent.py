...
"""
    ...
    async def extract_pdf(self, pdf_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"text": md_text.strip(), "pages": estimated_pages, "file_hash": file_hash}
    ...
    async def analyze_notification(
        ...
        logger.info(
            f"📍 Detected region: {region_info.get('region')} (confidence: {region_info.get('confidence')})"
        )
        ...
        for deadline in raw_deadlines:
            if deadline["type"] == "business_days":
                ...
                exact_date = self.deadline_calc.calculate_business_days(
                    start_date=notif_date, num_days=deadline["value"], region=region_name
                )
    ...
    def _extract_references(self, text: str, region_info: dict) -> list[dict[str, str]]:
        ...
        if not region_info.get("is_foral"):
            ...
            authority_name = self.region_detector.get_tax_authority_name(
                region_info["region"], True
            )
            authority_url = self.region_detector.get_tax_authority_url(region_info["region"], True)
            references.append(
                {
