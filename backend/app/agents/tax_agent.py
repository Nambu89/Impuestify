...
class TaxAgent:
    ...
    def _initialize(self):
        ...
        if self.api_key:
            ...
            logger.info(
                f"TaxAgent '{self.name}' initialized (year: {self.current_year}, IRPF fiscal: {self.irpf_fiscal_year}, quota: {self.autonomous_quota_year})"
            )
        else:
...
- Indica claramente cuál opción es más favorable y la diferencia exacta en euros."""
    ...
    def _filter_bad_responses(self, content: str, original_query: str) -> str:
        ...
        if is_permission_asking or (
            is_too_short_question and not any(c.isdigit() for c in content)
        ):
    ...
    async def run(
            ...
            try:
                ...
                if user_context.get("ingresos_brutos"):
                    context_parts.append(
                        f"Ingresos brutos anuales: {user_context['ingresos_brutos']}€"
                    )
                ...
                if user_context.get("facts"):
                    context_parts.append(
                        f"Hechos recordados: {'; '.join(user_context['facts'][:3])}"
                    )
        ...
        if fiscal_profile:
            ...
            if sit_laboral and sit_laboral.lower() in ("particular", "asalariado"):
                if roles_adicionales:
                    ...
                    fp_lines.append(
                        "- ⚠️ Este usuario tiene MÚLTIPLES roles fiscales. Considera TODAS sus fuentes de renta al responder."
                    )
                else:
                    fp_lines.append(
                        f"- ⚠️ Situación laboral: {sit_laboral} (NO es autónomo — NO usar herramientas de autónomos sin preguntar)"
                    )
            ...
            if fp_lines:
                is_autonomo_profile = (
                    sit_laboral.lower() in ("autónomo", "autonomo", "pluriactividad")
                    if sit_laboral
                    else False
                )
        ...
        try:
            ...
            if not moderation_result.is_safe:
                ...
                audit_logger.log_moderation_block(
                    user_id=user_id or "anonymous", categories=moderation_result.blocked_categories
                )

                # Return user-friendly block message
                block_message = llama_guard.get_block_message(moderation_result.blocked_categories)
                logger.warning(
                    f"🚫 Content blocked by Llama Guard: {moderation_result.blocked_categories}"
                )
        ...
        try:
            ...
            if cache_result.hit:
                ...
                logger.debug(
                    "Cache check: stale=%s, len=%d, preview='%s'",
                    is_stale,
                    len(cached_text),
                    cached_text[:80],
                )
        ...
        try:
            ...
            logger.info(
                f"🧠 Complexity Router: {complexity_result.level.value} → model={selected_model}, effort={reasoning_effort}, confidence={router_confidence:.2f}"
            )
        except ImportError:
        ...
        user_message = self._build_prompt(
            query, context, user_memory_context, fiscal_profile=fiscal_profile
        )
        ...
        try:
            ...
            logger.info(
                f"Calling OpenAI with model={selected_model}, tools={use_tools}, reasoning_effort={reasoning_effort}"
            )
            ...
            logger.info(
                f"OpenAI response - content length: {len(message.content) if message.content else 0}"
            )
            ...
            if message.tool_calls:
                ...
                for tool_call in message.tool_calls:
                    ...
                    if function_name in TOOL_EXECUTORS:
                        ...
                        if function_name == "update_fiscal_profile":
                            tool_result = await tool_executor(
                                user_id=user_id, db_client=db_client, **function_args
                            )
                        else:
                    ...
                    tool_response_content = tool_result.get(
                        "formatted_response", json.dumps(tool_result)
                    )
                ...
                try:
                    content = await self._stream_openai_response(
                        messages=messages, model=selected_model, progress_callback=progress_callback
                    )
            ...
            if not guardrails_system.validate_output_format(content):
                logger.warning(
                    "⚠️ Internal JSON-like pattern detected in response (not replacing — may be legitimate content)"
                )
    ...
    def _build_prompt(
        ...
        situacion = (
            (fiscal_profile or {}).get("situacion_laboral", "").lower() if fiscal_profile else ""
        )
        ...
        is_autonomo = (
            situacion in ("autónomo", "autonomo", "pluriactividad", "farmaceutico")
            or "creador_contenido" in fp_roles
            or "pluriactividad" in fp_roles
        )

        if any(
            kw in query_lower for kw in ["cuota", "cotiza", "autónomo", "autonomo", "pago como"]
        ):
