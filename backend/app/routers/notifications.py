...
async def analyze_notification(
    ...
    if not filename_valid:
        raise HTTPException(
            status_code=400, detail={"error": "Invalid filename", "details": filename_errors}
        )
    ...
    if validation_result.warnings:
        logger.warning(
            f"⚠️ File validation warnings for {file.filename}: {validation_result.warnings}"
        )

    logger.info(
        f"✅ File validated: {file.filename} ({validation_result.file_size / 1024:.2f}KB, hash: {validation_result.file_hash[:16]}...)"
    )
    ...
    try:
        ...
        logger.info(
            f"🔍 Document type: {detection['type']} (confidence: {detection['confidence']})"
        )

        # 🆕 STEP 3: Check if we should ask user
        if detector.should_ask_user(detection):
            logger.warning(
                f"⚠️ Low confidence ({detection['confidence']}), asking user for clarification"
            )
        ...
        if detection["type"] == "payslip":
            ...
            analysis = await agent.analyze_notification(
                pdf_path=temp_path, notification_date=notification_date
            )
        ...
        conversation = await conv_service.create_conversation(
            user_id=current_user.user_id, title=f"Notificación: {analysis['type']}"
        )
        ...
        return {"id": analysis_id, "conversation_id": conversation_id, **analysis}
...
async def get_notification_history(
    current_user: TokenData = Depends(get_current_user), limit: int = 10
):
    """
...
async def get_notification_analysis(
    analysis_id: str, current_user: TokenData = Depends(get_current_user)
):
    """
    ...
    **Response:**
    ...
    try:
        ...
        return {
            ...
            "region": {"region": row["region"], "is_foral": bool(row["is_foral"])},
...
async def delete_notification_analysis(
    ...
    **Path params:**
    ...
    try:
        ...
        # Delete
        await db.execute("DELETE FROM notification_analyses WHERE id = ?", [analysis_id])
            ...
            if date and days is not None:
                if days < 0:
                    deadline_lines.append(
                        f"{urgency_emoji} **{desc}**: vencido (hace {abs(days)} días)"
                    )
                elif days == 0:
                    deadline_lines.append(f"{urgency_emoji} **{desc}**: **HOY es el último día**")
                elif days <= 3:
                    deadline_lines.append(
                        f"{urgency_emoji} **{desc}**: {date} (quedan {days} días)"
                    )
                else:
