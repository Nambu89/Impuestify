...
async def get_db(request: Request) -> TursoClient:
    ...
    raise HTTPException(
        status_code=503, detail="Database not connected. Check Turso configuration."
    )
...
async def fts_search(db: TursoClient, query: str, k: int = 5) -> list[dict]:
    ...
    try:
        ...
        if region_info["confidence"] in ["high", "medium"]:
            ...
            logger.info(
                f"📍 Detected CCAA: {detected_ccaa} (confidence: {region_info['confidence']})"
            )
        ...
        if detected_foral:
            logger.info(
                f"📍 Detected Hacienda Foral: {detected_foral}, filtering for regional docs"
            )
            ...
            exclusion_parts = " AND ".join(
                ["LOWER(d.filename) NOT LIKE ?" for _ in foral_exclusions]
            )
        ...
        if (
            detected_ccaa
            and "Comunidad" not in detected_ccaa
            and detected_ccaa != "General (territorio común)"
        ):
...
"""
        ...
        if not relevant_chunks:
            ...
            return ImpuestifyResponse(
                ...
                processing_time=time.time() - start_time,
                conversation_id=conversation_id,
                metadata={"search_time": search_time, "chunks_found": 0, "search_method": "fts5"},
            )
        ...
        combined_context = (
            notification_context + rag_context if notification_context else rag_context
        )
        ...
        if not output_check.is_safe:
            ...
            answer = guardrails_system.apply_safety_wrapper(
                answer, risk_level=output_check.risk_level
            )
        ...
        logger.info(
            f"✅ Consulta procesada: {processing_time:.2f}s, {len(sources)} fuentes, conversation: {conversation_id}"
        )
        ...
        raise HTTPException(
            status_code=500, detail="Error interno al procesar la pregunta. Inténtalo de nuevo."
        )
