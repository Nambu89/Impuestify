...
class ModelDownloader:
    ...
    async def verify_all_models(self) -> Dict[str, bool]:
        ...
        required_models = [settings.embedding_model, settings.reranking_model]
...
class SystemValidator:
    ...
    def validate_pdf_files() -> Dict[str, Any]:
        ...
        if not pdf_dir.exists():
            return {"status": False, "message": f"Directorio {pdf_dir} no existe", "files": []}
    ...
    async def run_full_validation() -> Dict[str, Any]:
        ...
        results["overall_status"] = (
            results["openai_key"]
            and all(results["directories"].values())
            and results["pdf_files"]["status"]
            and all(results["models"].values())
        )
